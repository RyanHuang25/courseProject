...
'''
@Author : huangrenwu
@File: onlineSeleniumYK.py
@Time: 2022/6/24 14:46
@Email: leo.r.huang@microcore.tech
@Desc: 
'''


import time,platform,redis,json
...
class SeleniumSpider:
    ...
    def __init__(self):
        while True:
            self.account = pika.rpop('yc_account_file')
    ...
    def course_list(self):
        ...
        if self.course_count >= len(course_entrys):
            ...
            time.sleep(10)
            # self.bro.quit()
            pika.json('yc_account_done',json.dumps(self.account))
            return
